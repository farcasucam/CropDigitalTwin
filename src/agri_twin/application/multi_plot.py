...
class MultiPlotSimulation:
    ...
    def _evaluate(self, simulation_time: datetime) -> MultiPlotSimulationResult:
        ...
        for plot in self.plots:
            ...
            state = self._state_for(cycle, simulation_time)
            if status in {CropCycleStatus.ACTIVE, CropCycleStatus.HARVEST_READY}:
                ...
                self._states[cycle.crop_cycle_id] = state
        ...
        return MultiPlotSimulationResult(simulation_time, tuple(results), source)
    ...
    def _state_for(self, cycle: SyntheticCropCycle, instant: datetime) -> CropCycleState:
        status = self._status(cycle, instant)
        existing = self._states.get(cycle.crop_cycle_id)
